...
static void compatibilityWithListStack()
{
    std::cout << "=== Compatibility with std::stack<int, std::list<int> > ===" << std::endl;
    MutantStack<int, std::list<int> > mstack;
    mstack.push(10);
    mstack.push(20);
    mstack.push(30);
    std::stack<int, std::list<int> > s(mstack); 
    std::cout << "List-based stack top (expected 30): " << s.top() << std::endl;
    MutantStack<int, std::list<int> >::iterator it = mstack.begin();
    MutantStack<int, std::list<int> >::iterator ite = mstack.end();
    std::cout << "Iterating list-stack: ";
    while (it != ite)
    {
        std::cout << *it << " ";
        ++it;
