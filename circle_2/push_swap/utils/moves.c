...
/*   Updated: 2025/02/06 17:04:02 by root             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void perform_rot_rrr(t_list **head_a, t_list **head_b, t_list *from)
{
    ...
    while ((len_a - from->index) != 0 && (len_b - from->target_node->index) != 0)
    {
        reverse_rotate_all(head_a, head_b);
        len_a = list_size(*head_a);
        len_b = list_size(*head_b);
    }
    while ((len_a - from->index) != 0)
    {
        reverse_rotate_a(head_a);
    }
    while ((len_b - from->target_node->index) != 0)
    {
