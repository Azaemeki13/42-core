...
/*   Updated: 2024/09/23 11:04:59 by marvin           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int main(void) {
    ...
    printf("Comparing '%s' and '%s': %d\n", str1, str2, ft_memcmp(str1, str2, 5)); // Should return 0

    // Test 2: Different strings
    printf("Comparing '%s' and '%s': %d\n", str1, str3, ft_memcmp(str1, str3, 5)); // Should return -1

    // Test 3: Different strings, second is greater
    printf("Comparing '%s' and '%s': %d\n", str3, str1, ft_memcmp(str3, str1, 5)); // Should return 1

    // Test 4: Different lengths (first string is a prefix)
    printf("Comparing '%s' and '%s': %d\n", str1, str4, ft_memcmp(str1, str4, 5)); // Should return 0 (equal up to 5 chars)

    // Test 5: Compare different lengths (not enough bytes to compare)
    printf("Comparing '%s' and '%s': %d\n", str1, str4, ft_memcmp(str1, str4, 7)); // Should return -1 (str1 is less)
